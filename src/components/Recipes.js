...
const Recipes = () => {
    ...
    // Fetch recipes via serverless function
    const searchForRecipes = async (query) => {
        if (!query) return;
        setHideLoader(false);

        try {
            const response = await fetch(`/api/searchRecipes?query=${query}`);
            const returnedRecipes = await response.json();

            if (returnedRecipes.to === 0) {
                setShowNotificationTab(true);
            } else {
                const recipesData = getRequiredRecipesData(returnedRecipes.hits);
    ...
    // Scroll to recipes when new recipes appear
    useEffect(() => {
        if (recipes.length > 0) {
            recipesScroll.current.scrollIntoView({ behavior: 'smooth' });
        }
    ...
    // Scroll to healthy foods if navigated with query
    useEffect(() => {
        if (location.pathname === "/Recipes") window.scrollTo(0, 0);
        if (location.search === "?to=Healthy-foods") {
            healthyFoodsScroll.current.scrollIntoView();
    ...
    return (
        <main className="recipes">
            <SearchContainer searchForRecipes={searchForRecipes} hideLoader={hideLoader} />
            <HealthyFoods searchForRecipes={searchForRecipes} hideLoader={hideLoader} healthyFoodsScroll={healthyFoodsScroll} />
            {recipes.length > 0 && <RecipesList recipes={recipes} recipesScroll={recipesScroll} title="Recipes" />}
            {showNotificationTab && <NotificationTab text="No recipes found for your search" setShowNotificationTab={setShowNotificationTab} />}
        </main>
    );
};
...
const getRequiredRecipesData = (recipes) => {
    return recipes.map((recipe) => {
        const largeImage = recipe.recipe.images.LARGE
            ? recipe.recipe.images.LARGE.url
            : recipe.recipe.images.REGULAR.url;
