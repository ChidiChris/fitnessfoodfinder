...
const getCalorieCalculatorApiRequestUrl = (data) =>
    `https://fitness-calculator.p.rapidapi.com/dailycalorie?age=${data.age}&gender=${data.gender}&height=${data.height}&weight=${data.weight}&activitylevel=${data.activityLevel}`;
...
const getRequiredCalculationData = (data) => [
    { goal: 'Maintain weight', calory: Math.ceil(data['maintain weight']) },
    {
        goal: 'Mild weight loss',
        calory: Math.ceil(data['Mild weight loss'].calory),
        weight: `${data['Mild weight loss']['loss weight']} / week`
...
const CalorieCalculator = ({ setCalculationResult }) => {
    ...
    useEffect(() => {
        const savedCache = localStorage.getItem('calorieCalcCache');
        if (savedCache) {
            cache.current = new Map(JSON.parse(savedCache));
        }
    }, []);

    // Save cache to localStorage whenever it changes
    const saveCache = () => {
        localStorage.setItem('calorieCalcCache', JSON.stringify(Array.from(cache.current.entries())));
    };
    ...
    const handleCalculateSubmit = async (e) => {
        ...
        if (cache.current.has(cacheKey)) {
            ...
            setCalculationResult(cache.current.get(cacheKey));
            setShowLoader(false);
            e.target.reset();
            return;
        }
        ...
        try {
            ...
            if (!calculationResponse.ok) {
                throw new Error(`API error: ${calculationResponse.status}`);
            }
            ...
            if (calculationResult.status_code === 200) {
                ...
                // Store in cache
                cache.current.set(cacheKey, requiredCalculationData);
                saveCache();
            } else {
                alert('Bad request, please try again.');
            }
            ...
            console.error(error);
            alert('An error occurred while fetching data. Please check your internet connection and try again.');
        } finally {
            setShowLoader(false);
            e.target.reset();
        }
    ...
    return (
        <section className="calorie-calculator">
            ...
            <form className="calculator-container" onSubmit={handleCalculateSubmit}>
                ...
                <div className="calculator-buttons">
                    <button type="reset" disabled={showLoader}>Clear values</button>
                    <button type="submit" disabled={showLoader}>
                        <i className="fa-solid fa-circle-chevron-right" />
                        <span>{showLoader ? 'Calculating...' : 'Calculate'}</span>
                    </button>
                </div>
            </form>
...
const DropDownSelect = ({ options, name }) => (
    <select name={name}>
        {options.map((option, i) => (
            <option value={option.level} key={i}>
                {option.value}
            </option>
        ))}
    </select>
